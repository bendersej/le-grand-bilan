...
- `sources[].bot_walled: true` (optional) — marks a canonical official URL whose host blocks automated fetches (captcha/403), so it could not be machine-verified. The site does not render bot-walled sources; they are data-only, awaiting a human verification pass that removes the flag. At least one source per decision must be machine-verified (non-bot-walled), unless `missing_sources` applies
