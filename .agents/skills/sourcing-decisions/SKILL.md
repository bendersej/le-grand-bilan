...
- `senat.fr/scrutin-public/YYYY/scrNNNN.html` pages are curl-fetchable with exact vote tallies: the anchor for article 35 prolongation-of-intervention votes, paired with the legacy AN cri page.
...
- elysee.fr also hosts the FULL archive of past presidents' declarations under `/francois-hollande/YYYY/MM/DD/…` (~15k URLs in the sitemap, same date/keyword grep): the citable anchor for pre-2017 announcement decisions (abandon déchéance, Calais, non-candidature).
...
- CC decision URLs are case-sensitive across eras: 2013+ decisions accept uppercase (`2013666DC.htm`), but a decision numbered in an earlier year and published later 301s on uppercase — cite the lowercase form (`2012660dc.htm`).
