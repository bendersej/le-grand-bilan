...
- Sénat rapport synthesis PDFs (`senat.fr/rap/lXX-NNN/lXX-NNN-syn.pdf`, ~4 pages) are curl-fetchable and carry the headline measures of omnibus laws when the Légifrance fetch truncates: harvest the link from the dossier page (`grep 'rap/'`), never guess the rapport number.
